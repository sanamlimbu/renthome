...
func (r *userR) GetUploaderImages() ImageSlice {
	if r == nil {
		return nil
	}
	return r.UploaderImages
}

func (r *userR) GetIssueTokens() IssueTokenSlice {
	if r == nil {
		return nil
	}
	return r.IssueTokens
}

func (r *userR) GetManagerProperties() PropertySlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "first_name", "last_name", "email", "facebook_id", "google_id", "apple_id", "title", "description", "role", "mobile", "is_verified", "old_password_required", "avatar_id", "agency_id", "is_agency", "keywords", "created_at", "updated_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"role"}
	userColumnsWithDefault    = []string{"id", "first_name", "last_name", "email", "facebook_id", "google_id", "apple_id", "title", "description", "mobile", "is_verified", "old_password_required", "avatar_id", "agency_id", "is_agency", "keywords", "created_at", "updated_at", "deleted_at"}
...
// UploaderImages retrieves all the image's Images with an executor via uploader_id column.
func (o *User) UploaderImages(mods ...qm.QueryMod) imageQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"images\".\"uploader_id\"=?", o.ID),
	)
...
func (o *User) IssueTokens(mods ...qm.QueryMod) issueTokenQuery {
	...
	return IssueTokens(queryMods...)
}

// ManagerProperties retrieves all the property's Properties with an executor via manager_id column.
func (o *User) ManagerProperties(mods ...qm.QueryMod) propertyQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// LoadUploaderImages allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUploaderImages(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`images`),
		qm.WhereIn(`images.uploader_id in ?`, args...),
	...
	if singular {
		object.R.UploaderImages = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &imageR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UploaderID {
				local.R.UploaderImages = append(local.R.UploaderImages, foreign)
				if foreign.R == nil {
					foreign.R = &imageR{}
				}
...
// LoadManagerProperties allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadManagerProperties(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if singular {
		object.R.ManagerProperties = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &propertyR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ManagerID {
				local.R.ManagerProperties = append(local.R.ManagerProperties, foreign)
				if foreign.R == nil {
					foreign.R = &propertyR{}
				}
...
// AddUploaderImages adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UploaderImages.
// Sets related.R.Uploader appropriately.
func (o *User) AddUploaderImages(exec boil.Executor, insert bool, related ...*Image) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			UploaderImages: related,
		}
	} else {
		o.R.UploaderImages = append(o.R.UploaderImages, related...)
	}
...
// AddManagerProperties adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.ManagerProperties.
// Sets related.R.Manager appropriately.
func (o *User) AddManagerProperties(exec boil.Executor, insert bool, related ...*Property) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ManagerID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			ManagerProperties: related,
		}
	} else {
		o.R.ManagerProperties = append(o.R.ManagerProperties, related...)
	}
